...
extern "C" {
    ...
    pub fn vroom_g1_msm_auto_parallel(
        ctx: *mut c_void,
        points: *const c_void,
        scalars: *const c_void,
        npoints: usize,
    ...
    pub fn vroom_g1_msm_auto_parallel_matches_serial(
        ctx: *mut c_void,
        points: *const c_void,
        scalars: *const c_void,
        npoints: usize,
...
mod tests {
    ...
    impl TestFixture {
        ...
        fn msm_auto_parallel_matches_serial(&self, npoints: usize, num_threads: usize) -> bool {
            unsafe {
                vroom_g1_msm_auto_parallel_matches_serial(
                    self.ctx,
                    self.points,
                    self.scalars,
                    npoints,
    ...
    #[test]
    fn vroom_auto_parallel_matches_serial_sweep() {
        let _ = std::mem::size_of::<blst_p1>();
        ...
        for &k in &k_values {
            ...
            for &threads in &thread_candidates {
                ...
                let matches = fixture.msm_auto_parallel_matches_serial(n, threads);
                assert!(
                    matches,
                    "auto-parallel msm mismatch for npoints={n}, num_threads={threads}"
                );
            }
        }
    }
